...
export const IsometricMap: React.FC<IsometricMapProps> = ({
  ...
  useEffect(() => {
    ...
    bgGrad.addColorStop(0, '#DDD5C3');
    bgGrad.addColorStop(1, '#C8BFA8');
    ...
    // Soft vignette
    const vignette = ctx.createRadialGradient(cw2 / 2, ch2 / 2, cw2 * 0.28, cw2 / 2, ch2 / 2, cw2 * 0.75);
    ...
    for (let y = 0; y < sizeY; y++) {
      for (let x = 0; x < sizeX; x++) {
        ...
        // Draw 3D side blocks — deep slab like Dorfromantik
        const blockDepth = 20;
        ...
        // Inner top-left highlight (light coming from top-left — Dorfromantik/Terra Nil feel)
        if (selectedLayer === 'normal' || selectedLayer !== 'normal') {
          ctx.save();
          pathHexagon(ctx, screenX, screenY);
          ctx.clip();
          const hiGrad = ctx.createLinearGradient(screenX - w * 0.8, screenY - h, screenX + w * 0.5, screenY + h * 0.8);
  ...
  const getSidesColor = (t: TerrainType, layer: 'normal' | 'wrrl' | 'ffh' | 'flood', tile: TileData): string => {
    ...
    switch (t) {
      case 'Water': return '#2277A0';
      case 'Wiese': return '#72994C';
  ...
  const getDarkSidesColor = (t: TerrainType, layer: 'normal' | 'wrrl' | 'ffh' | 'flood', tile: TileData): string => {
    ...
    switch (t) {
      case 'Water': return '#185878';
      case 'Wiese': return '#547238';
